...
//essa  é uma função criadora de um tipo de ação
...
const cancelarContrato = (nome) => {
  //esse JSON que ela devolve é uma açao
  return {
...
const historicoDePedidoDeCashback = (
  historicoDePedidoDeCashbackAtual = [],
  acao
) => {
  //se a ação for CASHBACK, adicionamos o novo pedido à coleção existente
  if (acao.type === 'CASHBACK') {
    //uma cópia. Contém todos os dados existentes + o novo
    //não faça push
    return [...historicoDePedidoDeCashbackAtual, acao.dados];
  }
  //caso contrário, apenas ignoramos e devolvemos a coleção inalterada
  return historicoDePedidoDeCashbackAtual;
};
